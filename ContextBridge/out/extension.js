...
function activate(context) {
    ...
    context.subscriptions.push(vscode.window.onDidChangeActiveTextEditor(editor => {
        if (editor) {
            eventTracker.trackEvent("active_editor_changed", editor.document.fileName, editor.document.languageId, vscode.workspace.getWorkspaceFolder(editor.document.uri)?.name);
        }
    }));
    ...
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(e => {
        ...
        typingDebounceTimer = setTimeout(() => {
            eventTracker.trackEvent("typing_activity", e.document.fileName, e.document.languageId, vscode.workspace.getWorkspaceFolder(e.document.uri)?.name);
        }, 1000); // 1 second debounce
    }));
    // Track file saved
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(doc => {
        eventTracker.trackEvent("file_saved", doc.fileName, doc.languageId, vscode.workspace.getWorkspaceFolder(doc.uri)?.name);
    }));
    // Track debugging
...
class EventTracker {
    ...
    trackEvent(eventName, file, language, workspace) {
        const payload = {
            source: "vscode",
            event: eventName,
